...
import { data } from "jquery";
  ...
  const EditApplication = (props) => {
    ...
    const [loadAppliedError, setLoadAppliedError] = useState(false);
    const sleep = (ms) => {
  ...
  const handleRegistartion = (data) => {
    let load_applied = document.getElementById('input-load').value;
    let modified_date = document.getElementById('input-md').value;
    let ownership = document.getElementById('ownership').value;
    let status = document.getElementById('status').value;
    let category =  document.getElementById('category').value;
    let reviewer_id = document.getElementById('reviewer-id').value;
    let reviewer_comments = document.getElementById('reviewer-comments').value;
    let id = document.getElementById('input-application-id').value
    console.log(load_applied, modified_date,ownership,status,category,reviewer_id, reviewer_comments,id)
    let payload = {
      "id": id,
      "applicant": rowData.applicant.id,
  ...
  if (load_applied <=200){
    apiAxios.put('/view_applications/'+ id +'/', payload)
          .then(response => {
              if (response.status === 200) {
                  
                      console.log(response.data);
                      setRowData(...rowData, reviewer_id, ownership, category, load_applied, modified_date,status,reviewer_comments);
...
const handleLoadValueChange = (data) => {
  if (data.target.value > 200){
    setLoadAppliedError(true)
  }
  else{
    setLoadAppliedError(false);
  }
}
                ...
                <CardBody>
                  <Form onSubmit={handleSubmit(handleRegistartion)}>
                    ...
                    <h6 className="heading-small text-muted mb-4">
                      ...
                      <button type="button" style={{"float":"right"}} className="btn btn-dark">
                      <Link style={{"color" :"white"}}  to={{
                          pathname:'/admin/editApplicants',
                          idProps: {
                            data : rowData.applicant
                          }
                        }}>
                        Edit Applicant Details</Link>
                        </button>
                    </h6>
                    ...
                    <div className="pl-lg-4">
                      ...
                      <Col md="4">
                        ...
                        <Col md="4">
                          ...
                          {loadAppliedError > 0 && 
                <span className='error' style={{"color" :"red"}}>Load Applied should not be greater than 200kV</span>}
                        </Col>
                      </Row>
                      ...
                      <Row>
                        <Col lg="4">
                          <FormGroup>
                            ...
                            <select id="ownership" class="rounded form-control-alternative" style={{"height":"42px", "width":"100%"}} value={rowData.ownership} aria-label="Default select example">
                        ...
                        <Col lg="4">
                          <FormGroup>
                            ...
                            <select id="category" class="rounded form-control-alternative" style={{"height":"42px", "width":"100%"}} value={rowData.category} aria-label="Default select example">
                        ...
                        <Col lg="4">
                          <FormGroup>
                            ...
                            <select id="status" class="rounded form-control-alternative" style={{"height":"42px", "width":"100%"}} value={rowData.status} aria-label="Default select example">
                    ...
                    <h6 className="heading-small text-muted mb-4">
                      ...
                      <button type="button" style={{"float":"right"}} className="btn btn-dark">
                        <Link style={{"color" :"white"}}  to={{
                          ...
                          idProps: {
                            data : rowData.reviewer
                          }
                        }}>
                        Edit Reviewer Details</Link></button>
                    </h6>
                    <div className="pl-lg-4">
                      <Row>
                      <Col md="6">
                          <FormGroup>
                            ...
                            <select id="reviewer-id"  onChange={handleChange} class="rounded form-control-alternative" style={{"height":"42px", "width":"100%"}} value={reviwerId} aria-label="Default select example">
                            {
                              reviewerData.map((data)=>{
                                return(
